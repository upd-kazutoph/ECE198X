...
if opt.plot_interval != -1:
    for pair in transfer_combos:
        os.makedirs("out_train_cqt/%s/plot_%dt%d/" % (opt.model_name, pair[0], pair[1]), exist_ok=True)
        os.makedirs("out_train_cqt/%s/plot_%dt%d/" % (opt.model_name, pair[1], pair[0]), exist_ok=True)
