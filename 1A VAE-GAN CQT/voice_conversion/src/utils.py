...
def linear_to_cqt(signal):
    return librosa.cqt(
        signal, sr=sample_rate, fmin=librosa.note_to_hz('C2'), n_bins=60 * 2, bins_per_octave=12 * 2)
...
def amp_to_db(x):
    ...
    return 20 * np.log2(np.maximum(1e-5, x))
...
def cqt(y):
    ...
    cqt_kernel = zaf.cqtkernel(sample_rate, octave_resolution, minimum_frequency, maximum_frequency)

    # Compute the CQT spectrogram using the kernel
    time_resolution = 25
    return zaf.cqtspectrogram(y, sample_rate, time_resolution, cqt_kernel)
'''
...
def pre_emphasis(x):
    return lfilter([1, -pre_emphasis], [1], x)


def de_emphasis(x):
    return lfilter([1], [1, -pre_emphasis], x)
...
def reconstruct_waveform_cqt(cqt, n_iter=32):
    """Uses Griffin-Lim phase reconstruction to convert from a normalized
    cqt spectrogram back into a waveform."""
    denormalized = denormalize(cqt)
    amp_cqt = db_to_amp(denormalized)

    # Invert using Griffin-Lim
    wav = librosa.griffinlim_cqt(amp_cqt, sr=sample_rate, bins_per_octave=12*2,
        n_iter=n_iter, hop_length=80)
    return wav
...
def plot_batch_train(modelname, direction, curr_epoch, SRC, cyclic_SRC, fake_TRGT, real_TRGT):
    ...
    for src, cyclic_src, fake_target, real_target in zip(SRC, cyclic_SRC, fake_TRGT, real_TRGT):
        fname = "out_train_cqt/%s/%s/%s_%02d_%s.png"%(modelname, direction, direction, curr_epoch, i)
...
def plot_mel_transfer_infer(save_path, mel_in, mel_out):
    ...
    ax[0].imshow(np.real(mel_in), interpolation="None", aspect='auto')
    ax[0].set(title='Input')
