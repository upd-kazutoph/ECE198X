...
if opt.plot_interval != -1:
    for pair in transfer_combos:
        os.makedirs("out_train_mel/%s/plot_%dt%d/" % (opt.model_name, pair[0], pair[1]), exist_ok=True)
        os.makedirs("out_train_mel/%s/plot_%dt%d/" % (opt.model_name, pair[1], pair[0]), exist_ok=True)
...
def train_global():
    for epoch in range(opt.epoch, opt.n_epochs):
        ...
        lr_scheduler_G.step()
        for n in range(0, 2): # for n in range(0, opt.n_spkrs):
