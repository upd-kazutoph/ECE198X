...
import librosa
from utils import ls, preprocess_wav, melspectrogram, to_numpy, plot_mel_transfer_infer, reconstruct_waveform, reconstruct_waveform_cqt, cqt
...
parser.add_argument("--epoch", type=int, default=49, help="saved version based on epoch to test from")
parser.add_argument("--model_name", default="UNIT", type=str, help="name of the model")
parser.add_argument("--trg_id", default=2, type=str, help="id of the generator for the target domain")
parser.add_argument("--src_id", type=str, default=1, help="id of the generator for the source domain (Specify for a recon/cyclic evaluation with SSIM)")

parser.add_argument("--wav", type=str, default=False, help="path to wav file for input to transfer")
...
if opt.src_id: root += '_S%s_mel'%opt.src_id  # includes src_id if specified
...
def infer(S):
    ...
    S = S.view(1, 1, opt.img_width, opt.img_width)
    #S = S.view(S.size(0), S.size(0), opt.img_height, opt.img_width)
...
def audio_infer(wav):
    ...
    #spect_src = cqt(sample) # spectrogram output
    
    spect_src = spect_src[:opt.img_width]
    print("shape of sample = ",spect_src.shape)
    ...
    #spect_src.reshape(80,279)
    print("shape = (", spect_src.shape[0], ", ", spect_src.shape[1],")")
    ...
    if opt.src_id:
        ssim_recon.append(ssim(spect_src.real, spect_recon))
        ssim_cyclic.append(ssim(spect_src.real, spect_cyclic))

    # prepare file name for saving
    f = wav.split('\\')[-1] #WINDOWS
    #f = wav.split('/')[-1] #UBUNTU
    wavname = f.split('.')[0]
    ...
    #x = reconstruct_waveform_cqt(spect_trg)
...
if opt.wavdir:
    audio_files = glob.glob(os.path.normpath(os.path.join(opt.wavdir, '*.wav').replace("\\","/")))
    # print('length of audio_files =  '+ str(len(audio_files)))
    for i in enumerate(audio_files):
        print('file = ', str(i[1]))
        print('[File %d/%d]' % ((i[0])+1, len(audio_files)))
        audio_infer(i[1])
